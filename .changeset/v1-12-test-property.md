...
State is reset between runs (default: JSON-clone snapshot of `adapter.getState()` at start). A failing dispatch is treated as a property failure with the sequence preserved. Builds *on* the v1.7 sequence engine — no re-derivation. Hand-written equivalent: `docs/hand-written-test-property.md` (~60 lines). Consumer skill: `acture-test-property`.
